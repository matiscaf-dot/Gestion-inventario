...
def render():
    ...
    producto_sel = None
    if codigo_input:
        producto = df_filtrado[df_filtrado["Código de barras"].astype(str) == codigo_input]
        if not producto.empty:
            producto_sel = producto.iloc[0]
            st.write("📦 Producto encontrado:", producto_sel.get("Nombre producto", "Sin nombre"))
    ...
    if st.button("Actualizar precio"):
        if producto_sel is not None:
            try:
                supabase.table("inventario").update({"precio_producto": float(nuevo_precio)}).eq("codigo_proveedor", codigo_input).execute()
                st.success(f"✅ Precio actualizado para el producto con código {codigo_input}.")
